...
// producto = null; // Con esto el producto no existe y el catch manda el error
...
function getProduct(){ //con fetch
    let promise = fetch("https://fakestoreapi.com/products", { //ya es la promesa, llamada al servidor
        method: "GET"
    });
    
    promise.then( (response) => { //Cuando ya tenga la respuesta que te la de aquí
        response.json().then( (prods) => {//quiero los datos en formtato json , segunda promesa
                galleryProduct(prods);
                //crear Cards (prods);
                console.log("prods=>json()");
                console.log(prods);
            }//prods
        )//then json
        .catch( (err) => { //captura el error, el rejected de la promesa
...
getProduct()
/*              .then((prod)=>galleryProduct(prod))   //resolve
             .catch((err) => console.log(err.message));     //reject
 */
function galleryProduct(prods){
    let catalogue = '';
    catalogue
    product.forEach(function(imagen){
        catalogue += `
        ...
        <img src="${imagen.image}" class="card-img-top" alt="${imagen.id}">
         ...
         <p class="card-text">${imagen.description.slice(0,80)}...</p>
         <button type="button" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#exampleModal_${imagen.id}">Más info </button>
       </div>
     </div>
     <!-- Modal -->
     <div class="modal fade" id="exampleModal_${imagen.id}" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <div class="modal-dialog">
        <div class="modal-content">
        <div class="modal-header">
        <h1 class="modal-title fs-5" id="exampleModalLabel">${imagen.title}</h1>
        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
        </div>
        <div class="modal-body">
        ${imagen.description}
        <p class="text-end"><strong> $ ${imagen.price} USD </strong></p>
         </div>
    ...
    gallery.insertAdjacentHTML("beforeend",catalogue);
}
